import cv2
import numpy as np
from card_detection.current import augment
...
def eval_rank_suite(rank_suit_mapping, modelRanks, modelSuits):
    ...
    for rank, suit in rank_suit_mapping:
        ...
        bestSuitPredictions = model_wrapper.model_predict(
            modelSuits, suit, "suits"
        )  # min(suitDict, key=suitDict.get)
        bestRankPredictions = model_wrapper.model_predict(
            modelRanks, rank, "ranks"
        )  # min(rankDict, key=rankDict.get)

        # get the names and percentage of best and second best suits and ranks
        bestSuitName, bestSuitPer = model_wrapper.model_predictions_to_name(
            bestSuitPredictions
        )
        bestRankName, bestRankPer = model_wrapper.model_predictions_to_name(
            bestRankPredictions
        )
        sbestSuitName, sbestSuitPer = model_wrapper.model_predictions_to_name(
            bestSuitPredictions, loc=-2
        )
        sbestRankName, sbestRankPer = model_wrapper.model_predictions_to_name(
            bestRankPredictions, loc=-2
        )
...
def show_text(pred, four_corners_set, img):
    for i in range(0, len(pred)):
        ...
        for j in pred_list:
            cv2.putText(
                img, j, (startX, halfY + gap), font, 0.8, (50, 205, 50), 2, cv2.LINE_AA
            )
            gap += 30
