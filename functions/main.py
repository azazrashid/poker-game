...
@app.route('/poker_analyze', methods=['POST'])
def hand_analyze():
    ...
    try:
        cards = detect(file=image_file)
        hand = HandAnalyzer(cards).analyze(return_full_analysis=False,
                                           return_bestdisc_cnts=True)
        best_hand = list(hand.keys())[0]
        best_hand = best_hand.replace('X', '').replace('x', '')
        hand = card_name(best_hand)
        return jsonify({"Optimal Hand": hand})
...
def httpspoker(req: https_fn.Request) -> https_fn.Response:
